  ...
  test('cmp.in with [null] → throws', () => {
    ...
    assert.throws(
      () =>
        db
          .from(T)
          .where((c) => cmp.in(c.a, ['x', null as any] as any))
          .select((c) => ({ A: c.a }))
          .toSql(),
      (e: unknown) =>
        e instanceof QueryBuildError && /array contains null\/undefined/i.test(String(e)),
    );
  });
  ...
  test('NaN/Infinity → throws', () => {
    ...
    assert.throws(
      () =>
        db
          .from(T)
          .where((c) => cmp.eq(c.n as any, NaN as any))
          .select((c) => ({ N: c.n }))
          .toSql(),
    ...
    assert.throws(
      () =>
        db
          .from(T)
          .where((c) => cmp.eq(c.n as any, Infinity as any))
          .select((c) => ({ N: c.n }))
          .toSql(),
      (e: unknown) => e instanceof QueryBuildError && /NaN\/Infinity/i.test(String(e)),
    );
